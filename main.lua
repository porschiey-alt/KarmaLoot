...
local function initKarmaNotes(rosterIndex, rank, note) 
    local initialized = 0;
    for k = 1, tablelength(memberKarma), 1    
    do
...
local function main()
    ...
    optNeedButton:SetScript("OnEnter", function()
        toolTipText:SetText("|cFFFFFFFFROLL WITH KARMA");
        toolTip:Show();
    end);

    optNeedButton:SetScript("OnLeave", function() 
        toolTip:Hide();
    end);
    ...
    optGreedButton:SetScript("OnEnter", function()
        toolTipText:SetText("|cFFFFFFFFROLL WITHOUT ANY KARMA");
        toolTip:Show();
    end);

    optGreedButton:SetScript("OnLeave", function() 
        toolTip:Hide();
    end);
    ...
    optPassButton:SetScript("OnEnter", function()
...
local function klEarn(amount)
    ...
    if successFullGrants > 0 then
        SendChatMessage("All Raid members in " ..currentPlayer.name .."'s raid have earned ".. tostring(sanitizedAmt) .." Karma", "RAID");
    else
