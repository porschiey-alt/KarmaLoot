...
do
	...
	local SetHeight = function(self)
		self.frame:SetHeight( (self.displayRows * self.rowHeight) + 25);
		self:Refresh();
	end
		...
		for i = 1, num do
			...
			if not row then
				...
				if i > 1 then
					...
					row:SetPoint("TOPRIGHT", self.frame, "TOPRIGHT", -4, -20);
				end
	...
	local function SetDisplayCols (self, cols)
		...
		if not row then
			...
			--row:SetPoint("BOTTOMLEFT", self.frame, "TOPLEFT", 4, -18);
			--row:SetPoint("BOTTOMRIGHT", self.frame, "TOPRIGHT", -4, -18);
			row:Hide()
	...
	local function DoCellUpdate (rowFrame, cellFrame, data, cols, row, realrow, column, fShow, table, ...)
		if fShow then
			...
			cellFrame.text:SetFont("Interface/Addons/KarmaLoot/fonts/PTSansNarrow.ttf", 10)
    ...
    st.multiselection = multiselection or false
		...
		-- 		end
		-- 	end,
		-- 	["OnMouseUp"] = function(rowFrame, cellFrame, data, cols, row, realrow, column, table, button, ...)		-- LS: added "button" argument
		-- 		if button == "LeftButton" then	-- LS: only handle on LeftButton click (right passes thru)
		...
		t:SetPoint("TOPLEFT", f, "TOPLEFT", 7, -5)
		t:SetText("|cff00FF96Karma Leaderboard")
		t:SetFont("Interface/Addons/KarmaLoot/fonts/PTSansNarrow.ttf", 12)
    ...
    if multiselection then
      ...
      st.selected = multiselector
		...
		-- scrollframe.background = scrollframe:CreateTexture(nil, "BACKGROUND")
		-- scrollframe.background:SetAllPoints(scrollframe)
		-- scrollframe.background:SetColorTexture(0,0,0,1)
		...
		scrolltrough.background:SetColorTexture(0.05, 0.05, 0.05, .9);
		...
		scrolltroughborder.background:SetColorTexture(0,0,0, 1.0);
